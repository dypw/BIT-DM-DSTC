...
from Transformers.src.transformers.models.bert.modeling_bert import BertPreTrainedModel, BertModel, BertLMPredictionHead
...
class BertForCL(BertPreTrainedModel):
    ...
    def forward(self,
        ...
        if not self.is_eval:
            ...
            # loss = torch.pow(1 - score1[:, 0], 2) + torch.pow(score1[:, 1], 2)
            # loss = 0.3 - score1[:, 0] + score1[:, 1]
            # loss = loss * (loss > 0)
            alpha = ((1-score1[:, 0]).mean() + score1[:, 1].mean()).cpu().item()
            alpha = max(0, min(1, alpha))
            loss1 = torch.pow(1-score1[:, 0], self.pownum1) + torch.pow(score1[:, 1], self.pownum1)
            loss2 = torch.pow(1-score1[:, 0], self.pownum2) + torch.pow(score1[:, 1], self.pownum2)
            loss = (alpha * loss1 + (1 - alpha) * loss2) / (alpha * loss1 + (1 - alpha) * loss2).detach() * loss1.detach()      # rescale
            return loss.mean()
        else:
            score = self.output(sen_results.pooler_output)
