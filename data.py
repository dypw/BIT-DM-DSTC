...
class SimDataset(Dataset):
    ...
    def load_file(self, filename):
        with open(filename, 'r') as f:
            ...
            f.close()
            contents = [content[:-8] for content in contents]
        return contents
...
class SimcseDataCollator:
    ...
    def _encode(self, sentences: List[Example], pos_bili):
        ...
        for sentence in sentences:
            ...
            split_sentence = [aug2.augment(split_sentence[it], 1) if it != end else split_sentence[it]
                                  for it in range(len(split_sentence))]

            sen_num.append(len(split_sentence))
        ...
        for k in range(len(sen_num)):
            ...
            for i in range(start_index[k],end_index[k]+1):
                ...
                if it == cur_index + label_index[k]:
                    sen_type = 2
                    predict_flag = 1
                    if random.random()<(sen_num[k]/total_length)*4:
                        ...
                        neg_sen = self.neg_sampler.get_neg(all_sentences[it])
